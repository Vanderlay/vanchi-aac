...
class Tank_auth
{
	...
	function get_user()
	{
		return $this->ci->users->get_user_by_id($this->get_user_id(), 1);
	}
	 ...
	 * @return	array
	 */
	function create_user($username, $email, $password, $userAccess, $email_activation)
	{
	...
	{
		if($this->error) {
			foreach($this->error as $subject => $message) {
				switch($subject) {
					case 'username':
			...
			return "No errors occured. Proceeding...";
		}
	}
